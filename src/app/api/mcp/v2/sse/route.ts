/**
 * MCP v2 SSE Transport — /api/mcp/v2/sse
 *
 * Endpoints:
...
import { handleMcpV2SSE } from "../../../../../../open-sse/mcp-server/v2/httpTransport";
...
async function guardEnabled(): Promise<NextResponse | null> {
  ...
  if (transport !== "v2-sse") {
    return NextResponse.json(
      {
        error: `MCP transport is set to "${transport}", not "v2-sse". Change it from Settings.`,
      },
      { status: 400 }
    );
  }
